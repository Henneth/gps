...
class ReplayTracking_Model extends Model
{
	public static function getLocations($event_id, $datetime_from, $datetime_to) {
		$data = DB::select("SELECT gps_data.device_id AS device_id, datetime, unix_timestamp(datetime) AS timestamp, id, latitude_final, longitude_final, athletes.athlete_id, athletes.bib_number, first_name, last_name, country_code, colour_code, status
				FROM gps_data
