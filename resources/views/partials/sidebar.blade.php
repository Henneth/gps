...
<aside class="main-sidebar">
    ...
    <section class="sidebar">
        ...
        @if (empty($event_id))
            <?php $event_id = 0; $event_type = ""; $hideOthers = false; ?>
        @else
            @foreach ($events as $event)
                @if ($event->event_id == $event_id)
                    <?php $event_type = $event->event_type; $hideOthers = Auth::check() ? false : $event->hide_others; $event_name = $event->event_name;?>
                @endif
            @endforeach
        @endif
        ...
        <ul class="sidebar-menu">
            @if((!$hideOthers && live_event) || ($hideOthers && live_event == $event_id))
                ...
                @foreach ([['Live Tracking', 'fa-map-marker-alt']] as $item)
                    <li class="{{(Route::currentRouteName() == str_slug($item[0], '-') ) ? 'active' : ''}}"><a href="{{ url( 'event/' . live_event . '/' . str_slug($item[0], '-') ) }}"><i class='fa {{$item[1]}}'></i> <span>{{$item[0]}}</span></a></li>
                @endforeach
                @if (Auth::check())
                    @foreach ([['Draw Route', 'fa-pencil-alt'], ['Athletes', 'icon-directions_run'], ['Device Mapping', 'fa-exchange-alt'], ['Edit Event', 'fas fa-cog']] as $item)
                        <li class="{{(Route::currentRouteName() == str_slug($item[0], '-') && $event_id == live_event) ? 'active' : ''}}"><a href="{{ url( 'event/' . live_event . '/' . str_slug($item[0], '-') ) }}"><i class='fa {{$item[1]}}'></i> <span>{{$item[0]}}</span></a></li>
                    @endforeach
                @endif
            @endif
...
{{--             @if (!Request::is('view-all-events') && !Request::is('create-new-event') && !Request::is('home') && !Request::is('raw-data')) --}}
            ...
            @if (!$hideOthers || ($hideOthers && live_event != $event_id))
                ...
                @if(live_event != $event_id && $event_id != 0)
